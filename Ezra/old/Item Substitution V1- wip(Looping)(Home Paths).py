...
@author: Syed Haque
"""
from numpy import False_, float32, int64
import pandas as pd
from tqdm import tqdm

inv=pd.read_excel('V:/code/seal-team/Ezra/EZProductionInvSearchResults752.xlsx')
#inv.info()
sales=pd.read_excel('V:/code/seal-team/Ezra/EZ_Backlog_by_Product470.xlsx')
#sales.info()

df= r"V:/code/seal-team/Ezra/output/" # working dir for outputs
...
for sku in tqdm(skus):
    ...
    result_wip = children_sales_grouped.sort_values( by="BOM",inplace=True) # organizing by bom

    result= pd.concat([child_info_stack, children_sales_grouped], axis= 1)# adding the sku in quesiton info to our over all results
